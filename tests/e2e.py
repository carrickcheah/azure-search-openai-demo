...
def test_home(page: Page, live_server_url: str):
    ...
    expect(page).to_have_title("PR1MA AI Assistant")
...
def test_chat(sized_page: Page, live_server_url: str):
    ...
    page.goto(live_server_url)
    expect(page).to_have_title("PR1MA AI-Assistant")
    expect(page.get_by_role("heading", name="PR1MA AI-ASSISTANT")).to_be_visible()
    expect(page.get_by_role("button", name="Clear chat")).to_be_disabled()
    expect(page.get_by_role("button", name="Developer settings")).to_be_enabled()

    # Ask a question and wait for the message to appear
    page.get_by_placeholder("Type a new question (e.g. does xxxxxxxxxxannual eye exams?)").click()
    page.get_by_placeholder("Type a new question (e.g. does my bbbbb cover annual eye exams?)").fill(
        "Whats the ccccccc plan?"
    )
